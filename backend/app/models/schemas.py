...
class RagReference(BaseModel):
    """旅行计划中实际引用的知识源(用于前端展示)"""
    title: str
    source_type: str
    uri: str
    score: float = Field(default=0, description="相关度分数")
...
class TripPlan(BaseModel):
    ...
    rag_references: List[RagReference] = Field(default_factory=list, description="RAG引用来源")
...
class RagIngestTextRequest(BaseModel):
    """导入纯文本"""
    title: str = Field(default="", description="旅游攻略")
...
class RagIngestUrlRequest(BaseModel):
    """导入URL内容"""
    title: str = Field(default="", description="旅游攻略")
    url: str = Field(..., description="外部链接")


class RagSearchRequest(BaseModel):
    """检索请求"""
    query: str = Field(..., description="检索问题/关键词")
    top_k: int = Field(default=6, ge=1, le=20, description="返回条数")
...
class RagSearchItem(BaseModel):
    score: float
    source: RagSource
    chunk: RagChunkPublic


class RagIngestResponse(BaseModel):
    success: bool
    message: str = Field(default="")
    data: Optional[RagSource] = None


class RagSourcesResponse(BaseModel):
    success: bool
    message: str = Field(default="")
    data: List[RagSource] = Field(default_factory=list)


class RagSearchResponse(BaseModel):
    success: bool
    message: str = Field(default="")
    data: List[RagSearchItem] = Field(default_factory=list)
