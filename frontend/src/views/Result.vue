      ...
      <div class="side-nav">
        <a-affix :offset-top="80">
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            ...
            <a-menu-item key="budget" v-if="tripPlan.budge+t">
              <span>💰 预算明细</span>
            </a-menu-item>
            <a-menu-item key="map">
              <span>📍 景点地图</span>
            ...
            >
              <template v-if="Array.isArray(tripPlan.rag_references)">
                <a-alert
                  ...
                  show-icon
                  message="本次未命中知识库内容"
                  description="如果你已经导入了知识库但这里仍然为空，可能是内容与本次需求不相关，或本次生成使用了旧缓存。建议返回首页重新生成一次。"
                />

                <a-list v-else :data-source="tripPlan.rag_references" size="small">
                      ...
                      <div style="width: 100%">
                        <div style="display: flex; justify-content: space-between; gap: 12px; align-items: baseline;">
                          <div style="font-weight: 600; color: #333;">
                            {{ item.title }}
                            <span style="margin-left: 8px; color: #888; font-weight: 400;">({{ item.source_type }})</span>
                          </div>
                          <div style="color: #999; font-size: 12px;">score {{ item.score.toFixed(3) }}</div>
                        </div>
                        <div style="margin-top: 4px; color: #666; word-break: break-all;">
                          {{ item.uri }}
                        </div>
                      </div>
                    </a-list-item>
              ...
              <a-alert
                ...
                show-icon
                message="当前行程未包含引用信息"
                description="这通常是因为结果页加载的是旧版本 sessionStorage 数据。请返回首页重新生成行程。"
              />
            </a-card>
